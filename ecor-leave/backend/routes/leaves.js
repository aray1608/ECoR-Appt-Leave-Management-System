...
router.post('/add', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(400).json({ message: 'Failed to create leave', error: err.message });
  }
});
...
router.get('/upcoming', async (req, res) => {
  try {
    ...
    const leaves = await Leave.find({ from: { $gte: today } }).sort({ from: 1 });
    res.json(leaves);
  } catch (err) {
    res.status(500).json({ message: 'Failed to fetch upcoming leaves', error: err.message });
  }
});
...
router.get('/past', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to fetch past leaves', error: err.message });
  }
});
...
router.put('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(400).json({ message: 'Failed to update leave', error: err.message });
  }
});
