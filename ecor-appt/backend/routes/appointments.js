...
router.get('/upcoming', async (req, res) => {
  ...
  try {
    const appointments = await Appointment.find({ date: { $gte: today } }).sort({ date: 1 });
    res.json(appointments);
  } catch (error) {
    res.status(500).json({ message: 'Failed to fetch upcoming appointments', error });
  }
});
...
router.get('/past', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: 'Failed to fetch past appointments', error });
  }
});
...
router.put('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const updatedAppointment = await Appointment.findByIdAndUpdate(id, updatedData, {
      new: true,
    });
